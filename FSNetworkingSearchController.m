...
+ (void)searchForCategoryIdS:(NSDictionary *)parameters location:(CLLocation *)location  completion:(FSNSCVenuesBlock)completion{
    NSNumber * limit = [NSNumber numberWithInt:[[parameters objectForKey:@"limit"] intValue] ];
    NSNumber * radius = [NSNumber numberWithInt:[[parameters objectForKey:@"radius"] intValue] ];
    NSString * categoryIds = [parameters objectForKey:@"categoryId"];

    [self searchForCategoryId:categoryIds location:location radius:radius limit:limit intent:nil completion:completion];
}
+ (void)searchForQuery:(NSDictionary *)parameters location:(CLLocation *)location  completion:(FSNSCVenuesBlock)completion{
    NSNumber * limit = [NSNumber numberWithInt:[[parameters objectForKey:@"limit"] intValue] ];
    NSNumber * radius = [NSNumber numberWithInt:[[parameters objectForKey:@"radius"] intValue] ];
    NSString * query = [parameters objectForKey:@"query"];
    [self searchForQuery:query location:location radius:radius limit:limit intent:nil completion:completion];
}

+ (void)searchWithNoSuggestion:(NSString *)searchText location:(CLLocation *)location radius:(NSNumber *)radius limit:(NSNumber *)limit intent:(NSString *)intent completion:(FSNSCVenuesBlock)completion
{
    if (!self.sharedController.clientSecret && !self.sharedController.accessToken)
    {
        [NSException raise:NSStringFromClass(self) format:@"Your need to call first [FSNetworkingSearchController configureWithClientID:YOUR_CLIENT_ID clientSecret:YOUR_CLIENT_SECRET redirectURI:YOUR_REDIRECT_URI];"];
...
+ (void)suggestCompletionVenuesForQuery:(NSString *)query location:(CLLocation *)location radius:(NSNumber *)radius limit:(NSNumber *)limit completion:(FSNSCVenuesBlock)completion
{
    ...
    else if (self.sharedController.location)
    {
        [urlString appendFormat:@"&ll=%f,%f", self.sharedController.location.coordinate.latitude, self.sharedController.location.coordinate.longitude];
...
+ (void)searchForCategoryId:(NSString *)categoryId location:(CLLocation *)location radius:(NSNumber *)radius limit:(NSNumber *)limit intent:(NSString *)intent completion:(FSNSCVenuesBlock)completion
{
    self.sharedController.searchCompletion = completion;
    ...
    if (categoryId)
    {
        [urlString appendFormat:@"&categoryId=%@", categoryId];
    }
...
+ (NSMutableString *)authorizeURLString:(NSString *)urlString{
    NSMutableString * mutableUrlString = [NSMutableString stringWithFormat:urlString];
    [mutableUrlString appendFormat:@"?v=%@", FSNSCAPIVersioning];
    [mutableUrlString appendFormat:@"&m=%@", FSNSCAPIResponse];
    
    if (self.isLoggedIn)
    {
        [mutableUrlString appendFormat:@"&oauth_token=%@", self.sharedController.accessToken];
    }
    else
    {
        [mutableUrlString appendFormat:@"&client_id=%@", self.sharedController.clientID];
        
        [mutableUrlString appendFormat:@"&client_secret=%@", self.sharedController.clientSecret];
    }
...
+ (void)performSuggestCompletionVenuesQuery:(NSString *)urlString {
    ...
    self.sharedController.suggestCurrentConnection = [self.sharedController.communicator startWithUrl:url completionBlock:^(FSNConnection *connection) {
        ...
        {
            id object = connection.parseResult;
            self.sharedController.suggestCompletion(object[@"response"][@"minivenues"], connection.error);
        }
    }];
}
...
+ (void)performSearchQuery:(NSString *)urlString {
    ...
    self.sharedController.searchCurrentConnection = [self.sharedController.communicator startWithUrl:url completionBlock:^(FSNConnection *connection) {
        ...
        {
            id object = connection.parseResult;
            self.sharedController.searchCompletion(object[@"response"][@"venues"], connection.error);
        }
    }];
}

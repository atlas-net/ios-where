...
@objc open class JLToastView: UIView {
    ...
    init() {
        ...
        imageView.image = UIImage(named: "notificationToast")
    ...
    func updateView() {
        ...
        let imgViewWidth = (UIScreen.main.bounds.width * 28) / 75
        let imgViewHeight = imgViewWidth * 0.25
        let tableX = CGFloat(65.0)

        x = UIScreen.main.bounds.width  - tableX - imgViewWidth
        y = UIScreen.main.bounds.height * 0.5 + CGFloat(185)//局中

        self.frame = CGRect(x: x, y: y, width: imgViewWidth, height: imgViewHeight);
...
public extension JLToastView {
    fileprivate struct Singleton {
        static var defaultValues: [String: [UIUserInterfaceIdiom: AnyObject]] = [
            ...
            // textLabel.font
            JLToastViewFontAttributeName: [
                .unspecified: UIFont.systemFont(ofSize: 15),
                .phone: UIFont.systemFont(ofSize: 15),

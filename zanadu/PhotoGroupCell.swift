...
class PhotoGroupCell: UITableViewCell {
    ...
    func commomInitCell(_ group: PhotoGroupItem) {
        ...
        if  let phAsset = fetchResult.lastObject {
            ...
            PHImageManager().requestImage(for: phAsset, targetSize: CGSize(width: 160, height: 160), contentMode: .aspectFit, options: option, resultHandler: { (image, objects) in//获取相册的缩略图
        ...
        counts = fetchResult.count
//        print("文件夹中照片的数量    \(counts)")

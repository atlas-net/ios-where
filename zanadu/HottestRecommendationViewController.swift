//
//  HottestRecommendationViewController.swift
//  Atlas
//
...
import Foundation
import MJRefresh
class HottestRecommendationViewController: BaseTimeRecommendationTableViewController {
    //MARK: - Properties
   ...
   override func fetchData(){
        let recommendationQuery = DataQueryProvider.hotestRecommendationForLatestDate(nil, oldestDate: nil)
    ...
    override func fetchDataWithLatestDate(_ latestDate:Date){
        ...
        let recommendationQuery = DataQueryProvider.hotestRecommendationForLatestDate(latestDate, oldestDate: nil)
    ...
    override func fetchDataWithOldestDate(_ oldestDate:Date){
        let recommendationQuery = DataQueryProvider.hotestRecommendationForLatestDate(nil, oldestDate: oldestDate)
        recommendationQuery.executeInBackground { (objects:[Any]?, error) in
            self.loadingV.removeFromSuperview()
            self.tableView.mj_footer.endRefreshing()
            self.tableView.mj_header.endRefreshing()

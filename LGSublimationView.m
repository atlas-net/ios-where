...
-(void)setViewsToSublime:(NSArray *)viewsToSublime
{
    ...
    UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onCoverTapped:)];
...
-(void)setDescriptionLabelFont:(UIFont *)descriptionLabelFont
{
    ...
    for (UILabel*label in _descriptionLabelsArray) {
        ...
        UITapGestureRecognizer *singleTap =
        [[UITapGestureRecognizer alloc] initWithTarget:self
                                                action:@selector(onDescriptionTapped:)];
        [label addGestureRecognizer:singleTap];
    }
}
...
-(void)defaultSettings
{
    ...
    _pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed: 0.93 green:0.31 blue:0.49 alpha:1];
...
- (void)onCoverTapped:(UITapGestureRecognizer *)recognizer {
    if ([self.delegate respondsToSelector:@selector(onViewSelected:atIndex:)]) {
        [self.delegate onViewSelected:recognizer.view atIndex:self.currentPage - 1];
    }
}

- (void)onTitleTapped:(UITapGestureRecognizer *)recognizer {
    if ([self.delegate respondsToSelector:@selector(onViewSelected:atIndex:)]) {
        [self.delegate onViewSelected:recognizer.view atIndex:[_titleLabelsArray indexOfObject:recognizer.view]];
    }
}

- (void)onDescriptionTapped:(UITapGestureRecognizer *)recognizer {
    if ([self.delegate respondsToSelector:@selector(onViewSelected:atIndex:)]) {
        [self.delegate onViewSelected:recognizer.view atIndex:[_descriptionLabelsArray indexOfObject:recognizer.view]];
    }
}
